...
void collisionLoop()
{
    ...
    for (int i = 0; i < 6; i++)
    {
        camCollisions[i] = camera.gameObject.collisionBox[i];
    }
    ...
    for (int i = 0; i < gameObjectCount; i++)
    {
        if (!objectArray[i].collisionsEnabled){continue;}

        for (int d = 0; d < gameObjectCount; d++)
        {
            if (!objectArray[d].collisionsEnabled){continue;}
...
void goBackTransform(int i, int d, bool isCamera)
{
    if(objectArray[i].isTrigger || objectArray[d].isTrigger){return;}
    float dist[6];
